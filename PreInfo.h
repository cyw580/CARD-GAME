...
string Card::Intro(){
	...
	else if(func==48)return "接下去2回合 对方在每回合开始时恢复30HP";
	return "";
}
...
void previous(){
	...
	libcnt[6]=20;
	job[6]={400,4,40,5};
	lib[7][1]={"[意念控制]",2,45,0,0,0,38};//将对手牌库的一张牌清除特效后刷入牌库
	lib[7][2]={"[治疗光环]",2,0,40,0,0,39};//双方获得2回合治疗
	lib[7][3]={"[不完全记忆]",1,0,0,0,20,40};//将对手牌库的一张牌清除特效（和MISS）后放入手牌
	lib[7][4]={"[意念斗争]",3,90,0,20,0,38};
	lib[7][5]={"[意识反馈]",1,0,0,0,0,41};//此牌在手牌中时，你每出一张牌ATK+8(无论是否MISS)
	lib[7][6]={"[神圣意志]",4,0,140,50,0,42};//若在手牌中有4张[神圣意志]，直接赢得游戏
	lib[7][7]={"[清除杂念]",1,0,55,0,0,43};//刷1张[神圣意志]进入牌库
	lib[7][8]={"[唤醒意识]",2,0,80,15,0,44};//将随机一个手牌槽位变为[神圣意志]
	lib[7][9]={"[法力赋予]",3,10,60,30,0,45};//将对手牌库的一张牌特效改为"+4◆"后洗入牌库
	lib[7][10]={"[法力结晶]",3,0,50,0,0,46};//打出后回满费用且随机将对手一个槽位变为[法力结晶]
	lib[7][11]={"[禁锢思维]",2,0,30,0,0,47};//打出后随机将对手2张手牌 +1◆消耗
	libcnt[7]=11;
	job[7]={480,4,120,6};
}
...
void occ_func(int x){
	...
	if(x==1){
		...
		printf("\n\t<★信仰>每个标记额外增加4%抽到[神圣意志]的概率");printf("             ");SetColor(7);
		printf("\n\t 1.[虔诚] 手牌有4张[神圣意志]或有15个<★信仰>则赢得游戏");printf("           ");
		printf("\n\t 2.[奉献] 若回合结束时没有费用 则+1<★信仰>");printf("              ");
		printf("\n\t 3.[和平] 无法抽到公共牌库中ATK≥80的牌");printf("     ");
		printf("\n");printf("                                            ");
	}
	else if(x==8){
