...
int Card::Special(int from,int to){
	...
	// }
		...
		pl[from].heap[++pl[from].heapn]=lib[7][6];
	}
	else if(func==44){
		...
		int x=0,y=0;
		while(x==y) x=rand()%pl[to].cardcnt+1,y=rand()%pl[to].cardcnt+1;//选取随机槽位
		pl[to].handcard[x].cost+=1;
		pl[to].handcard[y].cost+=1;
	}
	return 0;
}
...
int Ask(int now){
	...
	for(int i=1;i<=pl[now].cardcnt;i++){
		if(pl[now].used[i]){
			if(pl[now].occ==7 && rand()%100<pl[now].buff[0]*4){
				pl[now].handcard[i]=lib[7][6];
				pl[now].used[i]=0;
				if(++pl[now].buff[10]>=4) {
					winner=now;
					return 1;
			...
			if(!option_use){
				...
				if(pl[now].used[cursor]){
					...
					if(pl[now].handcard[cursor].Use(now,3-now) && pl[now].handcard[cursor].func==40) {
						...
						if(pl[now].handcard[cursor].func==42) 
							if(++pl[now].buff[10]>=4) {
								winner=now;return 1;
							}
					}//牌的使用 与 [不完全记忆]
