...
int Card::Special(int from,int to){
	...
	if(func==1){
		...
		for(int i=1;i<=pl[to].cardcnt;i++){
			...
			if(pl[to].handcard[i].name=="[神圣意志]") pl[to].used[i]=1,pl[to].buff[0]+=2;
		}
	}
	else if(func==49){
		...
		for(int i=1;i<=pl[from].heapn;i++){
			while(pl[from].heap[i].name=="[虚空垃圾]" && i<=pl[from].heapn){
		...
		pl[from].used[++pl[from].cardcnt]=1;
		for(int i=1;i<=pl[from].heapn;i++){
			if(pl[from].heap[i].name=="[冲锋号令]") {
				pl[from].heap[i].name="[冲锋号令II]";
				pl[from].heap[i].cost=2;
				pl[from].heap[i].ATK=40;
			}
			if(pl[from].heap[i].name=="[有能狂怒]") {
				pl[from].heap[i].name="[有能狂怒II]";
				pl[from].heap[i].ATK=20;
			}
			if(pl[from].heap[i].name=="[生命果]") {
				pl[from].heap[i].name="[生命果II]";
				pl[from].heap[i].HEAL=80;
			}
			if(pl[from].heap[i].name=="[互惠共赢]") {
				pl[from].heap[i].name="[互惠共赢II]";
		...
		for(int i=1;i<=pl[from].heapn;i++){
			while(pl[from].heap[i].name=="[累赘]" && i<=pl[from].heapn){
		...
		pl[from].buff[0]+=2;
		pl[from].heap[++pl[from].heapn]=lib[7][libcnt[7]+1];
		pl[from].heap[++pl[from].heapn]=lib[7][libcnt[7]+5];
	}
	else if(func==66){
		pl[to].buff[6]=max(pl[to].buff[6],2);
	}
	...
	if(pl[now].occ==1 && (pl[now].cost==0 && pl[now].hp<=210)){
		for(int i=1;i<=pl[now].cardcnt;i++){
			if(pl[now].used[i]) {
				pl[now].used[i]=0;
				gettre[now]=1;
				pl[now].handcard[i]=lib[1][libcnt[1]+1];
				break;
			}
		}
		if(!gettre[now]){
			pl[now].handcard[rand()%pl[now].cardcnt+1]=lib[1][libcnt[1]+1];
		}
	}
	if(pl[now].occ==2 && pl[now].maxhp<=500){
		for(int i=1;i<=pl[now].cardcnt;i++){
			if(pl[now].used[i]) {
				pl[now].used[i]=0;
				gettre[now]=1;
				pl[now].handcard[i]=lib[2][libcnt[2]+1];
				break;
			}
		}
		if(!gettre[now]){
			pl[now].handcard[rand()%pl[now].cardcnt+1]=lib[2][libcnt[2]+1];
		}
	}
	if(pl[now].occ==3 && pl[now].hp<=150){
		for(int i=1;i<=pl[now].cardcnt;i++){
			if(pl[now].used[i]) {
				pl[now].used[i]=0;
				gettre[now]=1;
				pl[now].handcard[i]=lib[3][libcnt[3]+1];
				break;
			}
		}
		if(!gettre[now]){
			pl[now].handcard[rand()%pl[now].cardcnt+1]=lib[3][libcnt[3]+1];
		}
	}
	if(pl[now].occ==4 && pl[now].buff[0]>=6){
		for(int i=1;i<=pl[now].cardcnt;i++){
			if(pl[now].used[i]) {
				pl[now].used[i]=0;
				gettre[now]=1;
				pl[now].handcard[i]=lib[4][libcnt[4]+1];
				break;
			}
		}
		if(!gettre[now]){
			pl[now].handcard[rand()%pl[now].cardcnt+1]=lib[4][libcnt[4]+1];
		}
	}
	if(pl[now].occ==5 && pl[now].maxhp>=320 && pl[now].hp<=240){
		for(int i=1;i<=pl[now].cardcnt;i++){
			if(pl[now].used[i]) {
				pl[now].used[i]=0;
				gettre[now]=1;
				pl[now].handcard[i]=lib[5][libcnt[5]+1];
				break;
			}
		}
		if(!gettre[now]){
			pl[now].handcard[rand()%pl[now].cardcnt+1]=lib[5][libcnt[5]+1];
		}
	}
	if(pl[now].occ==6 && pl[now].buff[0]>=6){
		for(int i=1;i<=pl[now].cardcnt;i++){
			if(pl[now].used[i]) {
				pl[now].used[i]=0;
				gettre[now]=1;
				pl[now].handcard[i]=lib[6][libcnt[6]+3];
				break;
			}
		}
		if(!gettre[now]){
			pl[now].handcard[rand()%pl[now].cardcnt+1]=lib[6][libcnt[6]+3];
		}
	}
	if(pl[now].occ==7 && pl[now].buff[0]>=5 && pl[now].hp<=240){
		for(int i=1;i<=pl[now].cardcnt;i++){
			if(pl[now].used[i]) {
				pl[now].used[i]=0;
				gettre[now]=1;
				pl[now].handcard[i]=lib[7][libcnt[7]+4];
				break;
			}
		}
		if(!gettre[now]){
			pl[now].handcard[rand()%pl[now].cardcnt+1]=lib[7][libcnt[7]+4];
		}
	}
}
