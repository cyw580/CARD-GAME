...
void previous(){
	...
	libcnt[1]=11;
	fun[1][1][1]={"[熟练拾荒]",1,0,0,0,0,60};//宝藏
	funcnt[1][1]=1;
	job[1]={420,4,120,6};
	//caster
	...
	lib[2][10]=lib[2][9];//[过载]*2
	lib[2][11]={"[涅槃]",1,0,0,0,0,24};
	lib[2][12]=lib[2][3];//[凝结]*2
	libcnt[2]=12;
	fun[1][2][1]={"[活力精华]",1,1,70,0,0,61};//宝藏
	funcnt[2][1]=1;
	job[2]={600,4,80,5};
	//wizard
	...
	libcnt[5]=18;
	fun[1][5][1]={"[成长转型]",3,0,75,0,0,63};//宝藏
	funcnt[1][5]=1;
	job[5]={280,3,0,3};
	//demon
	lib[6][1]={"[虚空重生]",1,0,45,0,0,31};
    lib[6][2]={"[虚空休养]",2,0,70,0,0,33};
	lib[6][3]={"[暗能累积]",2,60,20,0,10,0};
	lib[6][4]={"[虚空魔法]",1,20,20,10,0,30};
    lib[6][5]={"[暗影炸弹]",5,220,30,30,0,29};
    lib[6][6]={"[虚空凝聚]",1,0,0,0,0,28};
    lib[6][7]={"[虚空之力]",2,95,0,0,0,30};
    lib[6][8]={"[虚空物质]",3,100,20,30,0,30};
    lib[6][9]={"[黑暗气息]",0,15,0,0,0,0};
    lib[6][10]={"[虚空能量]",4,160,25,10,0,27};
    lib[6][11]={"[暗能涌动]",3,90,40,0,10,0};
    lib[6][12]={"[暗能爆发]",4,130,35,0,10,29};
    lib[6][13]=lib[6][1];//[虚空重生]*2
    lib[6][14]={"[暗影术]",1,45,0,0,10,0};
    lib[6][15]=lib[6][3];//[暗能累积]*2
    lib[6][16]=lib[6][14];//[暗影术]*2
	lib[6][17]={"[黑洞吞噬]",3,90,50,0,0,33};
    libcnt[6]=17;
	lib[6][libcnt[6]+1]={"[虚空垃圾]",1,0,0,0,0,27};
	lib[6][libcnt[6]+2]={"[清理虚空]",2,0,0,0,0,32};
	fun[1][6][1]={"[激活虚空]",2,80,45,0,0,59};//宝藏
	funcnt[1][6]=1;
	job[6]={400,4,40,5};
	//minister
	lib[7][1]={"[说教]",1,30,0,0,0,38};//对手下回合[迷惑]
	lib[7][2]={"[治疗光环]",2,0,65,0,0,39};//双方获得2回合治疗
	lib[7][3]={"[不完全记忆]",1,0,20,0,15,40};//将对手牌库的一张牌清除特效和MISS后放入手牌
	lib[7][4]={"[意识混沌]",2,0,40,20,0,56};//对手下2回合[迷惑]
	lib[7][5]={"[意识反馈]",1,0,0,0,0,41};//此牌在手牌中时，你每出一张牌ATK+10(无论是否MISS)
	lib[7][6]={"[灵修]",2,10,70,0,0,43};//刷1张[神圣意志]进入牌库
	lib[7][7]={"[清除杂念]",1,0,40,0,0,43};
	lib[7][8]={"[冥想]",2,0,50,15,0,44};//将随机一个手牌槽位变为[神圣意志]
	lib[7][9]={"[法力赋予]",2,0,45,25,0,45};//将对手牌库的一张牌特效改为"+2◆"后洗入牌库
	lib[7][10]={"[法力结晶]",2,0,0,0,0,46};//打出后+4◆且随机将对手一个槽位变为[法力结晶]
	lib[7][11]={"[禁锢思维I]",2,0,60,10,0,47};//打出后随机将对手1张手牌 +1◆消耗
	lib[7][12]={"[禁锢思维II]",2,0,25,10,0,55};//打出后随机将对手2张手牌变为[激活思维]
	lib[7][13]={"[意识反馈]",1,0,0,0,0,41};
	lib[7][14]={"[圣洁洗礼]",3,40,55,0,0,49};//对手[虚弱]2回合
	libcnt[7]=14;
	lib[7][libcnt[7]+1]={"[神圣意志]",4,0,150,50,0,42};//若在手牌中有4张[神圣意志]，直接赢得游戏
	lib[7][libcnt[7]+2]={"[亵渎]",2,20,0,0,0,48};//清空对手手牌中[神圣意志]，每一张让对手+2<★信仰>
	lib[7][libcnt[7]+3]={"[激活思维]",0,0,15,0,0,0};
	fun[1][7][1]={"[坚守信念]",2,0,120,0,0,64};//宝藏
	fun[1][7][2]={"[精神控制]",0,0,0,0,20,65};//衍生牌
	funcnt[1][7]=2;
	job[7]={480,4,120,6};
}
