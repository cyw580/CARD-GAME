...
string Card::Intro(){
	...
	else if(func==57)return "-2<★疲惫>并升级牌库所有牌";
	else if(func==58)return "-3<★疲惫>";
	else if(func==59)return "-3<★原罪>,对手hp上限-80,清空牌库的[虚空垃圾]";
	else if(func==60)return "+2◆,手牌上限+1,增强部分牌库牌,使用后此牌变为[无中生有]";
	else if(func==61)return "+1<★牺牲>,+40hp上限";
	else if(func==62)return "+5<★法力>";
	else if(func==63)return "+2<★成长>,清空牌库中[累赘],将等量[累赘]刷入对手牌库";
	else if(func==64)return "+2<★信仰>,向牌库中刷入[神圣意志][精神控制]各1张";
	else if(func==65)return "随机选对手牌库一张牌,清除特效和MISS后视为你打出且不耗费";
	else if(func==66)return "";
	return "";
}
...
void previous(){
	...
	lib[0][5]={"[护甲]",1,0,0,35,0,0};
	lib[0][6]={"[治疗]",2,0,75,0,0,0};
	lib[0][7]={"[塔塔开]",2,95,0,0,0,0};
	lib[0][8]={"[正常的牌]",2,65,35,25,15,0};
	lib[0][9]={"[平凡的牌]",2,25,75,35,25,0};
	lib[0][10]={"[普通的牌]",3,120,45,20,25,0};
	lib[0][11]={"[回复膜法]",5,0,130,75,0,0};
	lib[0][12]={"[终结法术]",5,240,0,0,10,0};
	libcnt[0]=12;
	fun[2][0][1]={"[损坏的魔杖I]",1,0,0,0,0,51};
	fun[2][0][2]={"[损坏的魔杖II]",1,0,0,0,0,52};
	fun[2][0][3]={"[损坏的魔杖III]",1,0,0,0,0,53};
	fun[2][0][4]={"[损坏的魔杖IV]",1,0,0,0,0,54};
	funcnt[2][0]=4;
	//travaller
	lib[1][1]={"[冲锋号令]",3,45,0,0,0,1};
	lib[1][2]={"[生产建设]",3,0,40,25,0,2};
	lib[1][3]={"[互惠共赢]",1,0,0,0,0,3};
	lib[1][4]={"[燃起来了]",3,50,0,0,0,4};
	...
	libcnt[1]=11;
	lib[1][libcnt[1]+1]={"[熟练拾荒]",1,0,0,0,0,60};//宝藏
	job[1]={420,4,120,6};
	//caster
	lib[2][1]={"[灵魂协同]",1,55,-40,0,0,0};
	lib[2][2]={"[自我唤醒]",0,0,-25,0,0,0};
	lib[2][3]={"[凝结]",2,0,-75,0,0,1};
	lib[2][4]={"[点燃灵魂]",2,35,-45,0,0,50};
	lib[2][5]={"[觉醒]",1,0,-60,0,0,5};
	lib[2][6]={"[静养]",3,0,120,35,0,0};
	...
	libcnt[2]=12;
	lib[2][libcnt[2]+1]={"[活力精华]",1,1,70,0,0,61};//宝藏
	job[2]={600,4,80,5};
	//wizard
	lib[3][1]={"[点燃]",2,25,0,0,0,50};
	lib[3][2]={"[火焰弹]",0,45,0,0,30,0};
	lib[3][3]={"[炎爆]",7,330,0,0,0,0};
	lib[3][4]={"[投资]",1,0,0,0,0,12};
	lib[3][5]={"[法力屏障]",2,20,0,55,0,0};
	lib[3][6]={"[六边形法术]",6,135,35,30,0,12};
	lib[3][7]={"[血色怒火]",2,15,-45,0,0,5};
	lib[3][8]={"[生命腐蚀]",2,0,0,0,0,13};
	lib[3][9]={"[焕发活力]",2,0,35,0,0,12};
	lib[3][10]={"[会神一击]",1,65,0,0,0,0};
	lib[3][11]={"[不稳定元素]",2,145,45,0,40,35};
	lib[3][12]={"[火焰把戏]",3,185,0,0,20,36};//燃烧自己2回合
	lib[3][13]={"[急不可耐]",1,0,-20,0,0,37};//立即清空标记获得法力
	libcnt[3]=13;
	lib[3][libcnt[3]+1]={"[法力溢出]",2,0,100,0,0,62};//宝藏
	job[3]={320,4,120,7};
	//warrior
	lib[4][1]={"[坚守阵地]",2,0,0,95,0,0};
	lib[4][2]={"[挑衅]",1,0,50,55,0,14};
	lib[4][3]={"[挑衅]",1,0,50,55,0,14};
	lib[4][4]={"[冲刺]",1,80,0,0,15,0};
	lib[4][5]={"[莽撞]",2,135,0,0,20,0};
	lib[4][6]={"[莽撞]",2,135,0,0,20,0};
	lib[4][7]={"[背水一战]",0,0,0,0,0,15};
	lib[4][8]={"[持盾冲锋]",3,125,0,60,0,0};
	lib[4][9]={"[抛掷燃木]",4,125,-30,0,0,4};
	lib[4][10]={"[停战协议]",2,0,10,10,0,16};
	lib[4][11]={"[决斗]",4,245,0,0,10,0};
	lib[4][12]={"[绝对防御]",3,0,0,150,0,0};
	lib[4][13]={"[小型急救包]",2,0,55,30,0,0};
	lib[4][14]={"[小型急救包]",2,0,55,30,0,0};
	lib[4][15]={"[更新装备]",1,0,0,10,0,25};
	lib[4][16]={"[愤怒传递]",2,20,-30,0,0,17};
	lib[4][17]={"[休养生息]",2,0,50,0,0,58};
	libcnt[4]=17;
	lib[4][libcnt[4]+1]={"[提振士气]",1,0,0,0,0,57};//宝藏
	job[4]={500,4,240,6};
	//goblin
	lib[5][1]={"[毒雾弥漫]",2,0,0,0,0,6};
	lib[5][2]={"[采集]",1,0,10,0,0,18};
	lib[5][3]={"[发育]",1,0,0,0,20,19};
	lib[5][4]={"[回馈之击]",3,30,0,0,0,18};
	lib[5][5]={"[力量训练]",2,50,0,0,0,19};
	lib[5][6]={"[力量训练]",2,50,0,0,0,19};
	lib[5][7]={"[耐力训练]",2,0,50,0,0,19};
	lib[5][8]={"[防御训练]",2,0,0,40,0,19};
	lib[5][9]={"[全能训练]",3,40,35,25,0,19};
	lib[5][10]={"[全能训练]",3,40,35,25,0,19};
	lib[5][11]={"[潜伏攻击]",1,25,0,0,0,20};
	lib[5][12]={"[潜伏攻击]",1,25,0,0,0,20};
	lib[5][13]={"[潜伏攻击]",1,25,0,0,0,20};
	lib[5][14]={"[潜在危险]",3,80,0,0,0,20};
	lib[5][15]={"[正面交锋]",2,65,0,0,0,0};
	lib[5][16]={"[累赘]",4,0,0,0,0,26};
	lib[5][17]={"[发育]",1,0,0,0,20,19};
	lib[5][18]={"[快速发育]",3,0,35,0,0,34};
	libcnt[5]=18;
	lib[5][libcnt[5]+1]={"[成长转型]",3,0,75,0,0,63};//宝藏
	job[5]={280,3,0,3};
	//demon
	lib[6][1]={"[虚空重生]",1,0,35,0,0,31};
    lib[6][2]={"[虚空休养]",2,0,70,0,0,33};
	lib[6][3]={"[暗能累积]",2,60,20,0,10,0};
	lib[6][4]={"[虚空魔法]",1,20,20,10,0,30};
    lib[6][5]={"[暗影炸弹]",5,200,20,30,0,29};
    lib[6][6]={"[虚空凝聚]",1,0,0,0,0,28};
    lib[6][7]={"[虚空之力]",2,90,0,0,0,30};
    lib[6][8]={"[虚空物质]",3,100,15,30,0,30};
    lib[6][9]={"[黑暗气息]",0,15,0,0,0,0};
    lib[6][10]={"[虚空能量]",4,150,25,10,0,27};
    lib[6][11]={"[暗能涌动]",3,90,40,0,10,0};
    lib[6][12]={"[暗能爆发]",4,130,35,0,10,29};
    lib[6][13]={"[虚空重生]",1,0,35,0,0,31};
    lib[6][14]={"[暗影术]",1,45,0,0,10,0};
    lib[6][15]={"[暗能累积]",2,60,20,0,10,0};
    lib[6][16]={"[暗影术]",1,45,0,0,10,0};
	lib[6][17]={"[黑洞吞噬]",3,90,50,0,0,33};
    libcnt[6]=17;
	lib[6][libcnt[6]+1]={"[虚空垃圾]",1,0,0,0,0,27};
	lib[6][libcnt[6]+2]={"[清理虚空]",2,0,0,0,0,32};
	lib[6][libcnt[6]+3]={"[激活虚空]",2,80,20,0,0,59};//宝藏
	job[6]={400,4,40,5};
	//minister
	lib[7][1]={"[说教]",1,30,0,0,0,38};//对手下回合[迷惑]
	lib[7][2]={"[治疗光环]",2,0,55,0,0,39};//双方获得2回合治疗
	lib[7][3]={"[不完全记忆]",1,0,15,0,15,40};//将对手牌库的一张牌清除特效和MISS后放入手牌
	lib[7][4]={"[意识混沌]",2,0,30,20,0,56};//对手下2回合[迷惑]
	lib[7][5]={"[意识反馈]",1,0,0,0,0,41};//此牌在手牌中时，你每出一张牌ATK+10(无论是否MISS)
	lib[7][6]={"[灵修]",2,10,60,0,0,43};//刷1张[神圣意志]进入牌库
	lib[7][7]={"[清除杂念]",1,0,40,0,0,43};
	lib[7][8]={"[冥想]",2,0,40,15,0,44};//将随机一个手牌槽位变为[神圣意志]
	lib[7][9]={"[法力赋予]",2,0,40,25,0,45};//将对手牌库的一张牌特效改为"+2◆"后洗入牌库
	lib[7][10]={"[法力结晶]",2,0,0,0,0,46};//打出后+4◆且随机将对手一个槽位变为[法力结晶]
	lib[7][11]={"[禁锢思维]",2,0,45,10,0,47};//打出后随机将对手1张手牌 +1◆消耗
	lib[7][12]={"[禁锢思维]",2,0,20,10,0,55};//打出后随机将对手2张手牌变为[激活思维]
	lib[7][13]={"[意识反馈]",1,0,0,0,0,41};
	lib[7][14]={"[圣洁洗礼]",3,40,45,0,0,49};//对手[虚弱]2回合
	libcnt[7]=14;
	lib[7][libcnt[7]+1]={"[神圣意志]",4,0,160,60,0,42};//若在手牌中有4张[神圣意志]，直接赢得游戏
	lib[7][libcnt[7]+2]={"[亵渎]",2,20,0,0,0,48};//清空对手手牌中[神圣意志]，每一张让对手+2<★信仰>
	lib[7][libcnt[7]+3]={"[激活思维]",0,0,15,0,0,0};
	lib[7][libcnt[7]+4]={"[坚守信念]",2,0,100,0,0,64};//宝藏
	lib[7][libcnt[7]+5]={"[精神控制]",0,0,-30,0,20,65};
	job[7]={480,4,120,6};
}
...
void occ_treasure(int x){
	...
	if(x==1){
		...
		printf("宝藏获取条件:回合开始时HP≤150");printf("                   ");
	}
	else if(x==4){
		...
		printf("宝藏获取条件:回合开始时<★原罪>≥6");printf("               ");
	}
	else if(x==7){
		printf("宝藏获取条件:回合开始时<★信仰>≥5且HP≤240");printf("      ");
	}
	else if(x==8){
		printf("                                                     ");
	}
