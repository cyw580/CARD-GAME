...
string Card::Intro(){
	...
	else if(func==51)return "对方随机获得1-2回合随机buff";
	else if(func==52)return "自身随机获得1-2回合随机buff";
	else if(func==53)return "随机选择自身或对方,随机获得1-2回合随机buff";
	else if(func==54)return "自身和对方随机获得1-2回合随机buff";
	else if(func==55)return "打出后随机将对手2个手牌槽位变为[激活思维]";
	else if(func==56)return "对手下2回合[迷惑]";
	else if(func==57)return "若<★疲惫>标记不少于12个,-4<★疲惫>并升级牌库所有牌";
	else if(func==58)return "清空对手手牌中[神圣意志] 对手获得等量<★信仰>";
	else if(func==59)return "清空对手手牌中[神圣意志] 对手获得等量<★信仰>";
	else if(func==60)return "清空对手手牌中[神圣意志] 对手获得等量<★信仰>";
	return "";
}
...
void previous(){
	...
	funcnt[2][0]=4;
	fun[3][0][1]={"[损坏的魔杖I]",1,0,0,0,0,51};
	fun[3][0][2]={"[损坏的魔杖II]",1,0,0,0,0,52};
	fun[3][0][3]={"[损坏的魔杖III]",1,0,0,0,0,53};
	fun[3][0][4]={"[损坏的魔杖IV]",1,0,0,0,0,54};
	fun[3][0][5]={"[随机法术I]",1,0,0,0,0,0};
	fun[3][0][6]={"[随机法术II]",2,0,0,0,0,0};
	fun[3][0][7]={"[随机法术III]",3,0,0,0,0,0};
	fun[3][0][8]={"[随机法术IV]",4,0,0,0,0,0};
	funcnt[3][0]=8;
	//travaller
	...
	lib[4][16]={"[愤怒传递]",2,25,-30,0,0,17};
	libcnt[4]=16;
	lib[4][libcnt[4]+1]={"[提振士气]",1,0,0,0,0,57};
	job[4]={500,4,240,6};
	//goblin
	...
	lib[7][6]={"[灵修]",2,10,75,0,0,43};//刷1张[神圣意志]进入牌库
	lib[7][7]={"[清除杂念]",1,0,50,0,0,43};
	lib[7][8]={"[冥想]",2,0,45,15,0,44};//将随机一个手牌槽位变为[神圣意志]
	lib[7][9]={"[法力赋予]",2,0,55,25,0,45};//将对手牌库的一张牌特效改为"+2◆"后洗入牌库
	lib[7][10]={"[法力结晶]",2,0,0,0,0,46};//打出后+4◆且随机将对手一个槽位变为[法力结晶]
	lib[7][11]={"[禁锢思维]",2,0,50,10,0,47};//打出后随机将对手1张手牌 +1◆消耗
	lib[7][12]={"[禁锢思维]",2,0,20,10,0,55};//打出后随机将对手2张手牌变为[激活思维]
	lib[7][13]={"[意识反馈]",1,0,0,0,0,41};
	lib[7][14]={"[圣洁洗礼]",3,40,70,0,0,49};//对手[虚弱]2回合
	libcnt[7]=15;
	lib[7][libcnt[7]+1]={"[神圣意志]",4,0,140,50,0,42};//若在手牌中有4张[神圣意志]，直接赢得游戏
	lib[7][libcnt[7]+2]={"[亵渎]",2,20,0,0,0,48};//清空对手手牌中[神圣意志]，每一张让对手+2<★信仰>
	lib[7][libcnt[7]+3]={"[激活思维]",0,0,10,0,0,0};
	job[7]={480,4,120,6};
}
...
void occ_func(int x){
	...
	if(x==1){
		...
		printf("\n\t 1.[备战状态] 回合开始时-1<★疲惫>标记");printf("                ");
		printf("\n\t 2.[尽力出击] 使用攻击牌后+2<★疲惫>标记");printf("                    ");
		...
		printf("\n\t 1.[虔诚] 手牌中有4张[神圣意志]则直接赢得游戏");printf("               ");
