...
string occ_func(int x){
	if(x==1)return "属性: HP420 最大DEF100 手牌上限4 费用上限6\n1.所有职业技能牌都中规中矩，且都有特效";
	else if(x==2)return "属性: HP560 最大DEF80 手牌上限4 费用上限5\n1.职业标记：牺牲；每次因为自己出牌而受到伤害时会获得1个标记，每个标记使得攻击力增加5\n2.公共牌库中（回复量 ≥ 50）的牌不会抽到\n3.一些职业技能牌会通过削弱自己而获得优势";
	else if(x==3)return "属性: HP300 最大DEF100 手牌上限4 费用上限7\n1.职业标记：法力；每回合开始有30%获得1个标记，在下回合获得等同于标记数量的费用并将标记清空\n2.高伤害高爆发";
	else if(x==4)return "属性: HP480 最大DEF200 手牌上限4 费用上限6\n1.职业标记：疲惫；回合开始时标记清空，每回合第一次使用有攻击力牌后（无论是否miss）获得标记，标记存在时攻击力变为原来的60%\n2.每回合开始时若没有护甲 则护甲+30\n3.由于好战，公共牌库中所有有治疗效果的牌都不会抽到";
	else if(x==5)return "属性: HP280 最大DEF0 手牌上限4 费用上限3\n1.职业标记：成长；每个标记使你获得攻击力+10，HP上限+10，最大DEF+10，每次获得标记时恢复20HP\n2.每回合都有3费，且每使用一张牌（不计弃牌）会自动抽一张牌\n3.由于身体弱小，公共牌库中（ATK>20 或 DEF>30）的牌无法抽到，但身体敏捷，有穿透护盾攻击的能力";
	return "";
}
